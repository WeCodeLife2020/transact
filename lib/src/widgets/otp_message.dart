...
class _OtpMessageState extends State<OtpMessage> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){
        if(widget.stayOnPage==false)
          push(context, widget.route!);
        if(widget.stayOnPage==true)
          pop(context);
      },
      child: AlertDialog(


        insetPadding: EdgeInsets.symmetric(vertical: screenWidth(context,dividedBy: 7)),
          ...
          child: Column(
            children: [
              ...
              Row(
                ...
                children: [
                  Text(widget.title ?? " ",style: TextStyle(fontFamily: "sfProSemiBold",fontSize: 18,fontWeight: FontWeight.w600),),
                ],
              ),
